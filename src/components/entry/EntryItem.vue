<template>
  <div class="row fit  full-width ">
    ...
    <div class="col-3 text-right">
      <span v-show="item.indx == 1" class="label text-bold  q-pr-md">{{
        item.indx == 1 ? "Price" : ""
      }}</span>
      <q-input
        dense
    ...
    <div class="col text-right">
      <span
        v-show="item.indx == 1"
        class="label text-bold  q-pr-md"
        style="padding-right:60px"
        >{{ item.indx == 1 ? "Total" : "" }}</span
      >
      <q-input
        dense
        text-right
...
export default {
  ...
  computed: {
    getQty: {
      ...
      set(newqty) {
        this.item.item_qty = newqty < 0 ? 0 : newqty;
    ...
    getPrice: {
      ...
      set(_price) {
        this.item.price = _price < 0 ? 0 : _price;
        var qty = this.item.item_qty ? this.removeComma(this.item.item_qty) : 0;
        var price = this.item.price ? this.removeComma(this.item.price) : 0;
        this.item.Total = parseInt(qty) * parseInt(price);
        this.$emit("calculateTotal");
